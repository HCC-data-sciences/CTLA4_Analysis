---
title: "CTLA-4 Analysis in Testicular Germ Cell Tumors"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
```{r load_tgct_data, cache=TRUE, include=FALSE, eval=TRUE}
query.tgct.hg38 <- GDCquery(project = c("TCGA-TGCT"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.tgct.hg38)
raw.counts <- GDCprepare(query = query.tgct.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_tgct_data'}
...
write.fit(tfit,dt,file='results_testicular.txt')
```
...
no_repeat_tpm <- no_repeat_tpm %>% dplyr::select(-contains('-05A'))
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('TGCT','TGCT','TGCT','TGCT'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('TGCT','TGCT','TGCT','TGCT'))
```
...
write.table(combined, file='fdr_ipa_tables/testicular.tsv', row.names=FALSE, quote=FALSE, sep='\t')
