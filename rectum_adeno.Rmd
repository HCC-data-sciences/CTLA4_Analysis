---
title: "CTLA-4 Analysis in Rectum Adenocarcinoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
# Almost nothign here; only 1.2 log fold change for pdl1

```{r load_read_data, cache=TRUE, include=FALSE, eval=TRUE}
query.read.hg38 <- GDCquery(project = c("TCGA-READ"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.read.hg38)
raw.counts <- GDCprepare(query = query.read.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_read_data'}
...
write.fit(tfit,dt,file='results_rectum_adenocarcinoma.txt')
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('READ','READ','READ','READ'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('READ','READ','READ','READ'))
```
