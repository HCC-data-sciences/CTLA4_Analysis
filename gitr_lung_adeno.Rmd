---
title: "GITR Analysis in Lung Adenocarcinoma"
author: "Arjun Mittal"
date: "July 11, 2022"
output:
    html_document:
...
write.fit(tfit,dt,file='results_gitr_lung_adeno.txt')
```
...
group_res_consensus_tme <- deconvolute(by_group_joined, 'mcp_counter', indications=c('LUAD', 'LUAD', 'LUAD', 'LUAD'))
```
...
write.table(combined, file='fdr_ipa_tables/gitr_lung_adeno.tsv', row.names=FALSE, quote=FALSE, sep='\t')

```

```{r p_only}
p23 <- top_group2_vs_group3
p23$gene <- rownames(p23)
rownames(p23) <- NULL
p23 <- p23 %>% 
    dplyr::filter(abs(logFC) > 0.5, P.Value < 0.005) %>%
    dplyr::select(gene,logFC,P.Value)
names(p23) <- c('gene','LogFC_Group2_vs_Group3','P_Group2_vs_Group3')


p21 <- top_group2_vs_group1
p21$gene <- rownames(p21)
rownames(p21) <- NULL
p21 <- p21 %>% 
    dplyr::filter(abs(logFC) > 0.5, P.Value < 0.005) %>%
    dplyr::select(gene,logFC,P.Value)
names(p21) <- c('gene','LogFC_Group2_vs_Group1','P_Group2_vs_Group1')

combined <- bind_rows(p21,p23)
write.table(combined, file='new_ipa_table/gitr_lung_adeno.tsv', row.names=FALSE, quote=FALSE, sep='\t')
