---
title: "CTLA-4 Analysis in Pancreatic Adenocarcinoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
# Less than 1 log fold change difference in PDL1 between G1 and G2

```{r load_paad_data, cache=TRUE, include=FALSE, eval=TRUE}
query.paad.hg38 <- GDCquery(project = c("TCGA-PAAD"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.paad.hg38)
raw.counts <- GDCprepare(query = query.paad.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_paad_data'}
...
write.fit(tfit,dt,file='results_pancreatic.txt')
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('PAAD','PAAD','PAAD','PAAD'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('PAAD','PAAD','PAAD','PAAD'))
```
