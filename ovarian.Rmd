---
title: "CTLA-4 Analysis in Ovarian serous cystadenocarcinoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
# Only 1.2 fold log difference between group 1 and 2 for PDL1

```{r load_ov_data, cache=TRUE, include=FALSE, eval=TRUE}
query.ov.hg38 <- GDCquery(project = c("TCGA-OV"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.ov.hg38)
raw.counts <- GDCprepare(query = query.ov.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_ov_data'}
...
write.fit(tfit,dt,file='results_ovarian.txt')
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('OV','OV','OV','OV'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('OV','OV','OV','OV'))
```
...
write.table(combined, file='fdr_ipa_tables/ovarian.tsv', row.names=FALSE, quote=FALSE, sep='\t')
