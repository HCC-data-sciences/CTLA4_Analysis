---
title: "CTLA-4 Analysis in Breast Cancer"
author: "Arjun Mittal"
date: "June 23, 2022"
output:
    html_document:
...
```{r load_brca_data, cache=TRUE, include=FALSE, eval=FALSE}
query.brca.hg38 <- GDCquery(project = c("TCGA-BRCA"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
#GDCdownload(query.brca.hg38)
raw.counts <- GDCprepare(query = query.brca.hg38, summarizedExperiment = FALSE)

#head(raw.counts[1])
```

```{r readRaw}
raw.counts <- read.csv(file='raw.counts.brca.csv', sep=',', header=TRUE)
raw.counts <- raw.counts %>% dplyr::select(-X)
```
...
write.fit(tfit,dt,file='results_brca.txt')
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('BRCA','BRCA','BRCA','BRCA'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('BRCA','BRCA','BRCA','BRCA'))
```
...
write.table(combined, file='fdr_ipa_tables/breast.tsv', row.names=FALSE, quote=FALSE, sep='\t')
