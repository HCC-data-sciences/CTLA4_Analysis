---
title: "CTLA-4 Analysis in Lung Squamous Cell Carcinoma"
author: "Arjun Mittal"
date: "July 11, 2022"
output:
    html_document:
...
```{r load_lusc_data, cache=TRUE, include=FALSE, eval=TRUE}
query.lusc.hg38 <- GDCquery(project = c("TCGA-LUSC"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.lusc.hg38)
raw.counts <- GDCprepare(query = query.lusc.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_lusc_data'}
...
df.raw.counts <- df.raw.counts %>% dplyr::select(-contains('-02A'))
df.raw.counts <- df.raw.counts %>% dplyr::select(-contains('-02B'))
#df.raw.counts <- df.raw.counts %>% dplyr::select(-contains('-01B'))
#df.raw.counts <- df.raw.counts %>% dplyr::select(-contains('-01C'))
...
#cat('\n\n\n')
#cat('# TEST\n\n')
colnames(heatmap_frame) <- c('FOXP3', 'GITR', 'IDO1', 'LAG3', 'CSF1R', 'TIM3', 'CTLA4', 'PDL1')

heatmap_frame <- as.matrix(t(heatmap_frame))
set.seed(2)
Heatmap(heatmap_frame, name='Quartile Expression', column_title='Samples', 
...
write.fit(tfit,dt,file='results_lung_squamous.txt')
```
...
no_repeat_tpm <- no_repeat_tpm %>% dplyr::select(-contains('-2A'))
no_repeat_tpm <- no_repeat_tpm %>% dplyr::select(-contains('-2B'))
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('LUSC','LUSC','LUSC','LUSC'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('LUSC','LUSC','LUSC','LUSC'))
```
...
write.table(combined, file='fdr_ipa_tables/lung_squamous.tsv', row.names=FALSE, quote=FALSE, sep='\t')
