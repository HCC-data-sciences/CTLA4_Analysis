...
## No duplicated aliquots present in data!
...
#better_colors <- colorRampPalette(c("cyan", "deeppink3", 'red', 'orange'))           

cat('\n\n\n')
cat('### Quartile Expression of Genes of Interest Across Samples')
#heatmap(as.matrix(t(heatmap_frame)), Rowv = hclust_rows, Colv = hclust_cols, col=better_colors(4))

#cat('\n\n\n')
#cat('# TEST\n\n')
test_frame <- heatmap_frame
colnames(test_frame) <- c('FOXP3', 'GITR', 'IDO1', 'LAG3', 'CSF1R', 'TIM3', 'CTLA4', 'PDL1')
...
Heatmap(test_frame, name='Quartile Expression', column_title='Samples', 
        ...
        clustering_distance_columns='euclidean', show_column_names=FALSE)
```
...
#heatmap_better$Gene_Expression_Quartile <- factor(heatmap_better$Gene_Expression_Quartile)

#heatmap_better$Gene_Expression_Quartile <- revalue(heatmap_better$Gene_Expression_Quartile, c('4'='High Expression'))

#heatmap_better$Gene_Expression_Quartile <- revalue(heatmap_better$Gene_Expression_Quartile, c('3'='Intermediate - High Expression'))

#heatmap_better$Gene_Expression_Quartile <- revalue(heatmap_better$Gene_Expression_Quartile, c('2'='Intermediate - Low Expression'))

#heatmap_better$Gene_Expression_Quartile <- revalue(heatmap_better$Gene_Expression_Quartile, c('1'='Low Expression'))
...
#heatmap_ex <- ggplot(heatmap_better, aes(samples, gene)) + 
 ...
 #               scale_fill_manual(values=c('red2', 'orange', 'cadetblue2', 'blue4'), name='Quartile Groups')
...
```{r examine_diff_expressed, dependson='unweighted_differential_gene_expression', results='asis'}
...
cat('\n\n### High CTLA4 & Low PDL1 Group vs High CTLA4 & High PDL1 Group')
knitr::kable(head(top_group2_vs_group1[order(top_group2_vs_group1$adj.P.Val),],n=10))
cat('\n\n### High CTLA4 & Low PDL1 Group vs Low CTLA4 & Low PDL1 Group')
knitr::kable(head(top_group2_vs_group3[order(top_group2_vs_group3$adj.P.Val),],n=10))
cat('\n\n### High CTLA4 & High PDL1 Group vs Low CTLA4 & Low PDL1 Group')
...
```{r weighted_dge, cache=FALSE, eval=FALSE, include=FALSE}
...
```{r examine_weighted_dge, dependson='weighted_dge', results='asis', eval=FALSE, include=FALSE}
...
cat('\n\n### High CTLA4 & Low PDL1 Group vs High CTLA4 and High PDL1 Group')
knitr::kable(head(w_top_group2_vs_group1[order(w_top_group2_vs_group1$adj.P.Val),],n=10))
cat('\n\n### High CTLA4 & Low PDL1 Group vs Low CTLA4 and Low PDL1 Group')
knitr::kable(head(w_top_group2_vs_group3[order(w_top_group2_vs_group3$adj.P.Val),],n=10))
cat('\n\n### High CTLA4 & High PDL1 Group vs Low CTLA4 and Low PDL1 Group')
knitr::kable(head(w_top_group1_vs_group3[order(w_top_group1_vs_group3$adj.P.Val),],n=10))
```
...
```{r annotate_using_ENTREZID, eval=FALSE, include=FALSE}
...
```{r clusterProfiler, cache=TRUE, results='asis', fig.height=20, fig.width=15}
...
labels <- c('High CTLA4 & Low PDL1 Group vs High CTLA4 & High PDL1 Group', 'High CTLA4 & Low PDL1 Group vs Low CTLA4 & Low PDL1 Group', 'High CTLA4 & High PDL1 Group vs Low CTLA4 & Low PDL1 Group')
...
```{r deconv, results='asis', cache=TRUE, include=FALSE}
library(immunedeconv)

res_quantiseq <- deconvolute(no_repeat_tpm, 'quantiseq', tumor = TRUE)

res_mcp_counter <- deconvolute(no_repeat_tpm, 'mcp_counter')

colnames(res_mcp_counter) <- gsub('tpm_', '', colnames(res_mcp_counter))
#res_ciber <- deconvolute(no_repeat_tpm, 'cibsersort')

res_xCell <- deconvolute(no_repeat_tpm, 'xcell')
colnames(res_xCell) <- gsub('tpm_', '', colnames(res_xCell))

res_epic <- deconvolute(no_repeat_tpm, 'epic', tumor = TRUE)
colnames(res_epic) <- gsub('tpm_', '', colnames(res_epic))

res_abis <- deconvolute(no_repeat_tpm, 'abis')
colnames(res_abis) <- gsub('tpm_', '', colnames(res_abis))

cibersort_binary <- 'CIBERSORT.R'
cibersort_mat <- 'LM22.txt'
res_cibersort <- deconvolute(no_repeat_tpm, 'cibersort_abs')
colnames(res_cibersort) <- gsub('tpm_', '', colnames(res_cibersort))


res_timer <- deconvolute(no_repeat_tpm, 'timer', indications=replicate(368,'SKCM'))
colnames(res_timer) <- gsub('tpm_', '', colnames(res_timer))

res_consensus_tme <- deconvolute(no_repeat_tpm, 'consensus_tme', indications=replicate(368,'SKCM'))
colnames(res_consensus_tme) <- gsub('tpm_', '', colnames(res_consensus_tme))
```



```{r deconv_plotting, results='asis'}

cat("\n\n")
cat("## Whole Sample Comparision\n")
cat("### QuantiSeq Deconvolution")
res_quantiseq %>%
...
```{r quanti_seq_celltype_by_group, results='asis', eval=FALSE}
...
cat("\n\n\n\n\n")
cat('## QuantiSeq CellType Breakdown by Group')
cat('\n\n')
cat('### Group 1 - High CTLA-4 & High PDL1')
group1_cell %>%
    gather(sample, fraction, -cell_type) %>%
    ggplot(aes(x=sample, y=fraction, fill=cell_type)) + 
    geom_bar(stat = 'identity') + 
    coord_flip() + 
    scale_fill_brewer(palette = 'Paired') +
    scale_x_discrete(limits = rev(levels(group1_cell)))

cat('\n\n\n')
cat('### Group 2 - High CTLA-4 & Low PDL1')
group2_cell %>%
    gather(sample, fraction, -cell_type) %>%
    ggplot(aes(x=sample, y=fraction, fill=cell_type)) + 
    geom_bar(stat = 'identity') + 
    coord_flip() + 
    scale_fill_brewer(palette = 'Paired') +
    scale_x_discrete(limits = rev(levels(group2_cell)))


cat('\n\n\n')
cat('### Group 3 - Low CTLA-4 & High PDL1')
group3_cell %>%
    gather(sample, fraction, -cell_type) %>%
    ggplot(aes(x=sample, y=fraction, fill=cell_type)) + 
    geom_bar(stat = 'identity') + 
    coord_flip() + 
    scale_fill_brewer(palette = 'Paired') +
    scale_x_discrete(limits = rev(levels(group3_cell)))


cat('\n\n\n')
cat('### Group 4 - Low CTLA-4 & High PDL1')
group4_cell %>%
    gather(sample, fraction, -cell_type) %>%
    ggplot(aes(x=sample, y=fraction, fill=cell_type)) + 
    geom_bar(stat = 'identity') + 
    coord_flip() + 
    scale_fill_brewer(palette = 'Paired') +
    scale_x_discrete(limits = rev(levels(group4_cell)))



```

```{r group_breakdowns_summed, include=FALSE, eval=FALSE}
...
```{r deconvolute_group_totals, cache=TRUE, include=FALSE, eval=FALSE}
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('SKCM','SKCM','SKCM','SKCM'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('SKCM','SKCM','SKCM','SKCM'))
```
...
```{r group_deconv_plotting, results='asis', fig.height=10, fig.width=9, eval=FALSE}

cat("\n\n")
cat("## Grouped Sample Comparision\n")
cat("### QuantiSeq Results")
names(group_res_quantiseq) <- c('cell_type', 'High CTLA4 & High PDL1', 'High CTLA4 & Low PDL1', 'Low CTLA4 & Low PDL1', 'group4')
...
cat('\n\n\n\n')
cat('### EPIC Results')
names(group_res_epic) <- c('cell_type', 'High CTLA4 & High PDL1', 'High CTLA4 & Low PDL1', 'Low CTLA4 & Low PDL1', 'group4')
...
names(group_res_cibersort) <- c('cell_type', 'High CTLA4 & High PDL1', 'High CTLA4 & Low PDL1', 'Low CTLA4 & Low PDL1', 'group4')
...
cat("\n\n\n\n")
cat("### MCP Results")
names(group_res_mcp_counter) <- c('cell_type', 'High CTLA4 & High PDL1', 'High CTLA4 & Low PDL1', 'Low CTLA4 & Low PDL1', 'group4')
...
cat('\n\n\n\n')
cat('### TIMER Results')
names(group_res_timer) <- c('cell_type', 'High CTLA4 & High PDL1', 'High CTLA4 & Low PDL1', 'Low CTLA4 & Low PDL1', 'group4')
...
cat('\n\n\n\n')
cat('### ABIS Results')
names(group_res_abis) <- c('cell_type', 'High CTLA4 & High PDL1', 'High CTLA4 & Low PDL1', 'Low CTLA4 & Low PDL1', 'group4')
...
names(group_res_consensus_tme) <- c('cell_type', 'High CTLA4 & High PDL1', 'High CTLA4 & Low PDL1', 'Low CTLA4 & Low PDL1', 'group4')
...
```{r plotXCell, results='asis', fig.height=20, fig.width=12, eval=FALSE}
cat('\n\n\n\n')
cat('### xCell Results')
names(group_res_xCell) <- c('cell_type', 'High CTLA4 & High PDL1', 'High CTLA4 & Low PDL1', 'Low CTLA4 & Low PDL1', 'group4')
...
```{r moreTMB}
#calculated in this paper: https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7683772/#ST1
tmb <- read.csv(file='tmb_info_fronteirs_paper.csv', header=TRUE, sep=',')
group_info <- dge.counts$sample %>% dplyr::select(group)
group_info$sample <- gsub('unstranded_', '', rownames(group_info))
group_info$sample <- substring(group_info$sample,1,12)
rownames(group_info) <- NULL

my_samps <- tmb %>% dplyr::filter(sample %in% group_info$sample)
my_samps <- my_samps %>% filter(!is.na(TMB))

group_info <- group_info %>% dplyr::filter(sample %in% my_samps$sample)

tmb_matched_groups <- list(my_samps,group_info) %>% purrr::reduce(full_join, by='sample')
#350 matched this time!
#Group 1 is 124 
#Group 2 is 55
#Group 3 is 116
#Group 4 is 55
...
```{r plotTMB_Bao_Data, results='asis'}
library(rstatix)
library(ggpubr)
...
## TMB Analysis Using Values from Paper
N=350; Paper: https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7683772/

```{r plotTMB_from_paper, results='asis'}
library(rstatix)
library(ggpubr)
#g1 <- tmb_matched_groups %>% dplyr::filter(group == 1)
#g2 <- tmb_matched_groups %>% dplyr::filter(group == 2)
#g3 <- tmb_matched_groups %>% dplyr::filter(group == 3)

#avg1 <- mean(g1$TMB)
#se1 <- sd(g1$TMB)/sqrt(nrow(g1))
#avg2 <- mean(g2$TMB)
#se2 <- sd(g2$TMB)/sqrt(nrow(g2))
#avg3 <- mean(g3$TMB)
#se3 <- sd(g3$TMB)/sqrt(nrow(g3))

#per graphpad
#p comparing G1 to G2 is 0.0568
#p comapring G1 to G3 is 0.0698
#p comparing G2 to G3 is 0.0126


#use software instead
stat <- tmb_matched_groups %>% t_test(TMB ~ group)

#plot_tmb <- data.frame(group=c('High CTLA-4 & High PDL1','High CTLA-4 & Low PDL1','Low CTLA-4 & Low PDL1'),
 #                      TMB=c(avg1,avg2,avg3),
  #                     SE=c(se1,se2,se3)
   #                    )


#d <- ggplot(plot_tmb, aes(x = group, y=TMB)) + 
 #   geom_bar(stat='identity', fill='steelblue') + 
  #  geom_errorbar(aes(ymin=TMB-SE, ymax=TMB+SE), width=0.2, position=position_dodge(0.9)) +
   # theme_minimal()
#d

box <- tmb_matched_groups %>% dplyr::filter(group != 4)
#box <- box %>% dplyr::filter(TMB < 5000)
#box$TMB <- log10(box$TMB)
box$TMB <- log10(box$TMB)
box$group <- recode(box$group, 
...
Upon examination, the only upregulated gene in Group 2 compared to Group 3 relevant to DNA repair was ZNF365 used for HR in DS Breaks. That too only significant in non-adjusted P values


Still we decided to examine the relative differential expression of the DNA repair genes listed in the Wilms Tumor Paper, JITC 2022 amongst the 3 groups

```{r gene_from_paper_comparison, results='asis'}

cat('## Comparing Against the Paper DNA Repair Genes Found to be Activated\n\n')

#https://jitc.bmj.com/content/10/6/e004797#DC2
paper_genes <- c('MSH2', 'MSH6', 'POLE', 'EXO1', 'BLM', 'BRCA1', 'FANCA', 'MDC1', 'RAD18', 'RFC4', 'RFC5', 'TOPBP1', 'LIG3', 'PARP1', 'BRCA2', 'FANCG', 'FANCI', 'FANCD2', 'PRKDC')
compare_to_paper_23 <- top_group2_vs_group3 %>% dplyr::filter(gene_name %in% paper_genes)
compare_to_paper_13 <- top_group1_vs_group3 %>% dplyr::filter(gene_name %in% paper_genes)
compare_to_paper_21 <- top_group2_vs_group1 %>% dplyr::filter(gene_name %in% paper_genes)

cat('### High CTLA-4 & Low PDL1 vs High CTLA-4 & High PDL1\n')
knitr::kable(compare_to_paper_21)
cat('\n\n')

cat('### High CTLA-4 & Low PDL1 vs Low CTLA-4 & Low PDL1\n')
knitr::kable(compare_to_paper_23)
cat('\n\n')

cat('### High CTLA-4 & High PDL1 vs Low CTLA-4 & Low PDL1\n')
knitr::kable(compare_to_paper_13)
cat('\n\n')
#Found NO DIFFERENCE!

```

```{r dna_repar_paper_genes, include=FALSE}
genes_paper <- c('ACTL6A', 'ANKRD32', 'BLM', 'BRCA1', 
'BRCA2', 'BRIP1', 'CDCA5', 'CDK2', 'CHAF1A', 'CLSPN', 
'DNA2', 'DTL', 'EME1', 'EXO1', 'EYA1', 
'FANCA', 'FANCB', 'FANCD2', 'FANCG', 'FANCI', 'FOXM1', 
'GEN1', 'GINS4', 'HMGA2', 'KIF22', 'LIG3', 'MCM8', 'MMS22L', 
'MSH2', 'MSH6', 'NEIL3', 'PARP1', 'POLA1', 'POLE', 'POLQ', 'PRKDC', 'RAD18', 
'RAD54L', 'RFC4', 'RFC5', 'RMI1', 'SMARCAD1', 'SMC3', 
'TEX15', 'TOPBP1', 'TRIP13', 'UHRF1', 'WHSC1', 'XRCC2', 'ZRANB3')


compare_2_paper_23 <- top_group2_vs_group3 %>% dplyr::filter(gene_name %in% genes_paper)
compare_2_paper_13 <- top_group1_vs_group3 %>% dplyr::filter(gene_name %in% genes_paper)
compare_2_paper_21 <- top_group2_vs_group1 %>% dplyr::filter(gene_name %in% genes_paper)
...
write.table(combined, file='fdr_ipa_tables/melanoma.tsv', row.names=FALSE, quote=FALSE, sep='\t')

```

```{r CTLA4_Expression_Across_Groups, results='asis', fig.height=7, fig.width = 10}
cat('\n\n')
cat('# CTLA4 and PDL1 Scatter Plot by Group')
ctla4_pdl1_count <- lcpm[c('ENSG00000163599','ENSG00000120217'),]
rownames(ctla4_pdl1_count) <- c('CTLA4','PDL1')
ctla4_pdl1_count <- as.data.frame(t(ctla4_pdl1_count))
ctla4_pdl1_count$sample <- rownames(ctla4_pdl1_count)
rownames(ctla4_pdl1_count) <- NULL
ctla4_pdl1_count <- ctla4_pdl1_count %>% dplyr::select(sample,CTLA4,PDL1)
group_info <- as.data.frame(dge.counts$sample %>% dplyr::select(group)) %>%
    mutate(sample=rownames(dge.counts$sample)) %>%
    select(sample,group)
rownames(group_info) <- NULL

group_ctla4_pdl1 <- list(ctla4_pdl1_count, group_info) %>% purrr::reduce(full_join, by='sample')

scatter_ctla4 <- group_ctla4_pdl1

scatter_ctla4$group <- recode(scatter_ctla4$group, 
                    '1'='High CTLA-4 & High PDL1',
                    '2'='High CTLA-4 & Low PDL1',
                    '3'='Low CTLA-4 & Low PDL1',
                    '4'='Low CTLA-4 & High PDL1'
...
ggplot(scatter_ctla4, aes(x=PDL1, y=CTLA4)) +
...
## Overall
- Only two genes were upregulated in Group 2 (High CTAL4 & Low PDL1) 
  compared to Group 1 (High CTLA4 & High PDL1): NECAB and BMP7

- These were not significantly upregulated compared to Group 3 
  (Low CTLA4 & Low PDL1) expression however

- The only gene differentially expressed between Group 2 and Group 3 
  was an over expression of CTLA4... Which is the whole distinction 
  between the groups in any case.

- If we use non-adjusted P-values, ClusterProfiler showed the DNA damage repair 
  pathway was activated in Group 2 compared to Group 3
  - Previous work done by our group shows TMB should be decreased in conditions with high DNA damage repair
  - TMB analysis instead showed, that Group 2 had on average, a higher TMB out of 
    any condition
  - Further upon examining the DNA repair genes found in a recent paper by Dr. Bao to be activated in Wilms Tumors, no expression difference was found
