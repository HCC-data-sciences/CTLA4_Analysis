---
title: "CTLA-4 Analysis in Head and Neck Squamous Cell Carcinoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
```{r load_hnsc_data, cache=TRUE, include=FALSE, eval=TRUE}
query.hnsc.hg38 <- GDCquery(project = c("TCGA-HNSC"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.hnsc.hg38)
raw.counts <- GDCprepare(query = query.hnsc.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_hnsc_data'}
...
write.fit(tfit,dt,file='results_head_and_neck.txt')
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('HNSC','HNSC','HNSC','HNSC'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('HNSC','HNSC','HNSC','HNSC'))
```
...
write.table(combined, file='fdr_ipa_tables/head_neck.tsv', row.names=FALSE, quote=FALSE, sep='\t')
