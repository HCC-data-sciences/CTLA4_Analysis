---
title: "CTLA-4 Analysis in Liver hepatocellular carcinoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
```{r load_lihc_data, cache=TRUE, include=FALSE, eval=TRUE}
query.lihc.hg38 <- GDCquery(project = c("TCGA-LIHC"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.lihc.hg38)
raw.counts <- GDCprepare(query = query.lihc.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_lihc_data'}
...
write.fit(tfit,dt,file='results_liver.txt')
```
...
no_repeat_tpm <- no_repeat_tpm %>% dplyr::select(-contains('-02A'))
no_repeat_tpm <- no_repeat_tpm %>% dplyr::select(-contains('-02B'))
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('LIHC','LIHC','LIHC','LIHC'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('LIHC','LIHC','LIHC','LIHC'))
```
...
write.table(combined, file='fdr_ipa_tables/liver.tsv', row.names=FALSE, quote=FALSE, sep='\t')
