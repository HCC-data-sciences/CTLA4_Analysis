---
title: "CTLA-4 Analysis in Kidney renal clear cell carcinoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
# PDL1 only 1 fold lower in Group 2 compared to Group 1

```{r load_kirc_data, cache=TRUE, include=FALSE, eval=TRUE}
query.kirc.hg38 <- GDCquery(project = c("TCGA-KIRC"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.kirc.hg38)
raw.counts <- GDCprepare(query = query.kirc.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_kirc_data'}
...
df.raw.counts <- df.raw.counts %>% dplyr::select(-contains('5633-01B'),
                                                 -contains('A4W0-05A'),
                                                 -contains('3923-01B'),
                                                 -contains('3924-01B'),
                                                 -contains('5635-01B')
...
write.fit(tfit,dt,file='results_kidney_renal_clear_cell.txt')
```
...
no_repeat_tpm <- no_repeat_tpm %>% dplyr::select(-contains('5633-01B'),
                                                 -contains('A4W0-05A'),
                                                 -contains('3923-01B'),
                                                 -contains('3924-01B'),
                                                 -contains('5635-01B')
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('KIRC','KIRC','KIRC','KIRC'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('KIRC','KIRC','KIRC','KIRC'))
```
