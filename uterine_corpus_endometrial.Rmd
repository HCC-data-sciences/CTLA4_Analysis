---
title: "CTLA-4 Analysis in Uterine Corpus Endometrial Carcinoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
## Only 1.4 fold log change between PDL1 in g1 and g2

```{r load_ucec_data, cache=TRUE, include=FALSE, eval=TRUE}
query.ucec.hg38 <- GDCquery(project = c("TCGA-UCEC"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.ucec.hg38)
raw.counts <- GDCprepare(query = query.ucec.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_ucec_data'}
...
df.raw.counts <- df.raw.counts %>% dplyr::select(-contains('A139-02A'),
                                                 -contains('A139-01C'),
                                                 -contains('A0CC-01B'),
                                                 -contains('A0CA-01B'),
                                                 -contains('A26L-01C')
...
write.fit(tfit,dt,file='results_uterine_corpus.txt')
```
...
no_repeat_tpm <- no_repeat_tpm %>% dplyr::select(-contains('A139-02A'),
                                                 -contains('A139-01C'),
                                                 -contains('A0CC-01B'),
                                                 -contains('A0CA-01B'),
                                                 -contains('A26L-01C')
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('UCEC','UCEC','UCEC','UCEC'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('UCEC','UCEC','UCEC','UCEC'))
```
