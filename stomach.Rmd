---
title: "CTLA-4 Analysis in Stomach Adenocarcinoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
```{r load_stad_data, cache=TRUE, include=FALSE, eval=TRUE}
query.stad.hg38 <- GDCquery(project = c("TCGA-STAD"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.stad.hg38)
raw.counts <- GDCprepare(query = query.stad.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_stad_data'}
...
write.fit(tfit,dt,file='results_stomach.txt')
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('STAD','STAD','STAD','STAD'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('STAD','STAD','STAD','STAD'))
```
...
write.table(combined, file='fdr_ipa_tables/stomach.tsv', row.names=FALSE, quote=FALSE, sep='\t')
