---
title: "CTLA-4 Analysis in Lymphoid Neoplasm Diffuse Large B-cell Lymphoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
```{r load_dlbc_data, cache=TRUE, include=FALSE, eval=TRUE}
query.dlbc.hg38 <- GDCquery(project = c("TCGA-DLBC"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.dlbc.hg38)
raw.counts <- GDCprepare(query = query.dlbc.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_dlbc_data'}
...
write.fit(tfit,dt,file='results_dlbc.txt')
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('DLBC','DLBC','DLBC','DLBC'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('DLBC','DLBC','DLBC','DLBC'))
```
...
write.table(combined, file='fdr_ipa_tables/dlbc.tsv', row.names=FALSE, quote=FALSE, sep='\t')
