---
title: "CTLA-4 Analysis in Acute Myeloid Leukemia"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
```{r load_laml_data, cache=TRUE, include=FALSE, eval=TRUE}
query.laml.hg38 <- GDCquery(project = c("TCGA-LAML"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.laml.hg38)
raw.counts <- GDCprepare(query = query.laml.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_laml_data'}
...
The genes of interest in our study were: CTLA4, FOXP3, GITR (TNFRSF18), IDO1, LAG3, CSF1R, TIM3 (HAVCR2), and PDL1 (CD274). IDO1 not present
...
list_of_all_gene_quartiles <- list(test2, test3, test5, test6, test7, test8, test9)
...
colnames(test_frame) <- c('FOXP3', 'GITR', 'LAG3', 'CSF1R', 'TIM3', 'CTLA4', 'PDL1')
...
master <- master %>% subset(select=c(-quartile_foxp3,-quartile_gitr,-quartile_lag3,-quartile_csf1r,-quartile_tim3))
...
write.fit(tfit,dt,file='results_acute_myeloid_leukemia.txt')
```
