---
title: "GITR Analysis in Bladder Urothelial Carcinoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
write.fit(tfit,dt,file='results_gitr_bladder.txt')
```
...
no_repeat_tpm <- no_repeat_tpm %>% dplyr::select(-contains('A0C8-01B'),
                                                 -contains('A13J-01B'),
                                                 -contains('A13I-01B')
                                                 )
```
...
group_res_consensus_tme <- deconvolute(by_group_joined, 'mcp_counter', indications=c('BLCA', 'BLCA', 'BLCA', 'BLCA'))
```
...
write.table(combined, file='fdr_ipa_tables/gitr_bladder.tsv', row.names=FALSE, quote=FALSE, sep='\t')
...
write.table(combined, file='new_ipa_table/gitr_bladder.tsv', row.names=FALSE, quote=FALSE, sep='\t')
