---
title: "CTLA-4 Analysis in Prostate Adenocarcinoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
## Note Group 1 only expresses PDL1 about 1 log fold more 

```{r load_prad_data, cache=TRUE, include=FALSE, eval=TRUE}
query.prad.hg38 <- GDCquery(project = c("TCGA-PRAD"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.prad.hg38)
raw.counts <- GDCprepare(query = query.prad.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_prad_data'}
...
df.raw.counts <- df.raw.counts %>% dplyr::select(-contains('-06A'))
df.raw.counts <- df.raw.counts %>% dplyr::select(-contains('8265-01B'), 
                                                 -contains('7740-01B'),
                                                 -contains('8258-01B')
                                                 )
...
write.fit(tfit,dt,file='results_prostate_adenocarcinoma.txt')
```
...
no_repeat_tpm <- no_repeat_tpm %>% dplyr::select(-contains('-06A'))
no_repeat_tpm <- no_repeat_tpm %>% dplyr::select(-contains('8265-01B'), 
                                                 -contains('7740-01B'),
                                                 -contains('8258-01B')
                                                 )
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('PRAD','PRAD','PRAD','PRAD'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('PRAD','PRAD','PRAD','PRAD'))
```
