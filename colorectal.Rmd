---
title: "CTLA-4 Analysis in Colorectal Cancer"
author: "Arjun Mittal"
date: "June 23, 2022"
output:
    html_document:
...
query.colo.hg38 <- GDCquery(project = c("TCGA-COAD"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.colo.hg38)
raw.counts <- GDCprepare(query = query.colo.hg38, summarizedExperiment = FALSE)
...
write.fit(tfit,dt,file='results_colorectal.txt')
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('COAD','COAD','COAD','COAD'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('COAD','COAD','COAD','COAD'))
```
...
```{r matchingToCOAD, results='asis'}

file_coad=readLines('COAD.clin.merged.txt') 
test <- read.table(textConnection(file_coad[[791]]))
test$V1 <- NULL

test2 <- read.table(textConnection(file_coad[[800]]))
test2$V1 <- NULL
colnames(test2) <- NULL

colnames(test) <- test2
colnames(test) <- toupper(colnames(test))
colnames(test) <- substring(colnames(test),1,12)


group_info_3 <- dge.counts$sample %>% dplyr::select(group)
group_info_3$sample <- gsub('unstranded_','',rownames(group_info_3))
group_info_3$sample <- substring(group_info_3$sample,1,12)
rownames(group_info_3) <- NULL

holder <- colnames(test)
colnames(test) <- NULL
test <- t(test)
colnames(test) <- c('MSI_Value')
test <- as.data.frame(test)
test$sample <- holder

msi_group <- list(group_info_3, test) %>% purrr::reduce(full_join, by='sample')
msi_group <- msi_group %>% drop_na(group)
msi_group <- msi_group %>% drop_na(MSI_Value)
msi_group$MSI_Value <- factor(msi_group$MSI_Value)
levels(msi_group$MSI_Value) <- c('indeterminate', 'msi-high', 'msi-low', 'ms-stable')
#791
#800

box <- msi_group %>% dplyr::filter(group != 4)
box$group <- recode(box$group, 
                    '1'='High CTLA-4 & High PDL1',
                    '2'='High CTLA-4 & Low PDL1',
                    '3'='Low CTLA-4 & Low PDL1',
...
#pdf('test_msi')
ggplot(box, aes(x=group, fill=MSI_Value)) +
