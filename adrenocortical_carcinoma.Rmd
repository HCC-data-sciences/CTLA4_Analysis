---
title: "CTLA-4 Analysis in Adrenocortical Carcinoma"
author: "Arjun Mittal"
date: "June 23, 2022"
output:
    html_document:
...
```{r load_brca_data, cache=TRUE, include=FALSE, eval=TRUE}
query.acc.hg38 <- GDCquery(project = c("TCGA-ACC"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.acc.hg38)
raw.counts <- GDCprepare(query = query.acc.hg38, summarizedExperiment = FALSE)
...
## No duplicated aliquots present in data! Checked and verified
...
interest_list <- dge.counts[dge.counts$genes$gene_name %in% c('FOXP3','IDO1', 'LAG3', 'CSF1R', 'HAVCR2', 'CD274'),]
```

## Determining Expression of 8 Genes of Interest Across the Samples

The genes of interest in our study were: FOXP3, IDO1, LAG3, CSF1R, TIM3 (HAVCR2), and PDL1 (CD274).
...
list_of_all_gene_quartiles <- list(test2, test4, test5, test6, test7,test9)
...
colnames(test_frame) <- c('FOXP3', 'IDO1', 'LAG3', 'CSF1R', 'TIM3', 'PDL1')
...
write.fit(tfit,dt,file='results_acc.txt')
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('ACC','ACC','ACC','ACC'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('ACC','ACC','ACC','ACC'))
```
