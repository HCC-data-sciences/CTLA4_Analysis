---
title: "CTLA-4 Analysis in Kidney Chromophobe Carcinoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
```{r load_kich_data, cache=TRUE, include=FALSE, eval=TRUE}
query.kich.hg38 <- GDCquery(project = c("TCGA-KICH"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.kich.hg38)
raw.counts <- GDCprepare(query = query.kich.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_kich_data'}
...
set.seed(2)
pdf('kidney_chromophobe_heatmap')
...
write.fit(tfit,dt,file='results_kidney_chromophobe.txt')
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('KICH','KICH','KICH','KICH'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('KICH','KICH','KICH','KICH'))
```
