---
title: "CTLA-4 Analysis in Uveal Melanoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
# No GITR, IDO1, or CTLA4


```{r load_uvm_data, cache=TRUE, include=FALSE, eval=TRUE}
query.uvm.hg38 <- GDCquery(project = c("TCGA-UVM"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.uvm.hg38)
raw.counts <- GDCprepare(query = query.uvm.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_uvm_data'}
...
write.fit(tfit,dt,file='results_uveal_melanoma.txt')
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('UVM','UVM','UVM','UVM'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('UVM','UVM','UVM','UVM'))
```
