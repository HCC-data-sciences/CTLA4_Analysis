---
title: "GITR Analysis in Uterine Carcinosarcoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
```{r load_ucs_data, cache=TRUE, include=FALSE, eval=TRUE}
query.ucs.hg38 <- GDCquery(project = c("TCGA-UCS"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.ucs.hg38)
raw.counts <- GDCprepare(query = query.ucs.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_ucs_data'}
...
write.fit(tfit,dt,file='results_gitr_uterine_carcinosarcoma.txt')
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('UCS', 'UCS', 'UCS', 'UCS'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'mcp_counter', indications=c('UCS', 'UCS', 'UCS', 'UCS'))
```
...
write.table(combined, file='new_ipa_table/gitr_uterine_carcine.tsv', row.names=FALSE, quote=FALSE, sep='\t')
