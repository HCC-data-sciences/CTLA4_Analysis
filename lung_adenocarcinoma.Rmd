---
title: "CTLA-4 Analysis in Lung Adenocarcinoma"
author: "Arjun Mittal"
date: "July 11, 2022"
output:
    html_document:
...
```{r load_luad_data, cache=TRUE, include=FALSE, eval=TRUE}
query.luad.hg38 <- GDCquery(project = c("TCGA-LUAD"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.luad.hg38)
raw.counts <- GDCprepare(query = query.luad.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_luad_data'}
...
write.fit(tfit,dt,file='results_lung_adeno.txt')
```
...
no_repeat_tpm <- no_repeat_tpm %>% dplyr::select(-contains('-01B'))
no_repeat_tpm <- no_repeat_tpm %>% dplyr::select(-contains('-01C'))
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('LUAD','LUAD','LUAD','LUAD'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('LUAD','LUAD','LUAD','LUAD'))
```
...
write.table(combined, file='fdr_ipa_tables/lung_adenocarcinoma.tsv', row.names=FALSE, quote=FALSE, sep='\t')
