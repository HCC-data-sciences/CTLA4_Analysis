---
title: "CTLA-4 Analysis in Lower Grade Glioblastoma"
author: "Arjun Mittal"
date: "June 23, 2022"
output:
    html_document:
...
```{r load_lgg_data, cache=TRUE, include=FALSE, eval=TRUE}
query.lgg.hg38 <- GDCquery(project = c("TCGA-LGG"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.lgg.hg38)
raw.counts <- GDCprepare(query = query.lgg.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_lgg_data'}
...
df.raw.counts <- df.raw.counts %>% dplyr::select(-contains('-11'))
df.raw.counts <- df.raw.counts %>% dplyr::select(-contains('-02'))
#convert df to edgeR list
dge.counts <- DGEList(as.matrix(df.raw.counts))

```

## No duplicated aliquots present in data! Done 
...
list_of_all_gene_quartiles <- list(test2, test5, test6, test7, test9)
...
colnames(test_frame) <- c('FOXP3', 'LAG3', 'CSF1R', 'TIM3', 'PDL1')
...
Heatmap(test_frame, name='Quartile Expression', column_title='Samples', 
        ...
        clustering_distance_columns='euclidean', show_column_names=FALSE, k=2)
dev.off()
```
...
group1 <- master %>% filter(quartile_pdl1 >=3 & quartile_ctla4 >= 3) #group1, N=210
group1 <- group1 %>% subset(select=c(sample_names))

group2 <- master %>% filter(quartile_pdl1 <=2 & quartile_ctla4 >= 3) #group2, N=50
group2 <- group2 %>% subset(select=c(sample_names))

group3 <- master %>% filter(quartile_pdl1 <=2 & quartile_ctla4 <= 2) #group3, N=211
group3 <- group3 %>% subset(select=c(sample_names))

group4 <- master %>% filter(quartile_pdl1 >=3 & quartile_ctla4 <=2) #group 4, N=50
...
write.fit(tfit,dt,file='results_gbm.txt')
```
...
```{r clusterProfiler, cache=TRUE, results='asis', fig.height=20, fig.width=15, eval=TRUE}
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('GBM','GBM','GBM','GBM'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('GBM','GBM','GBM','GBM'))
```
...
```{r TMB_analysis_original, eval=FALSE, include=FALSE}
...
```{r plotTMB_Bao_Data, results='asis', eval=FALSE}
library(rstatix)
library(ggpubr)
...
```{r plotTMB_from_paper, results='asis', eval=FALSE}
library(rstatix)
library(ggpubr)
...
```{r gene_from_paper_comparison, results='asis', eval=FALSE}
...
```{r dna_repar_paper_genes, include=FALSE, eval=FALSE}
