---
title: "CTLA-4 Analysis in Esophageal Carcinoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
```{r load_esca_data, cache=TRUE, include=FALSE, eval=TRUE}
query.esca.hg38 <- GDCquery(project = c("TCGA-ESCA"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.esca.hg38)
raw.counts <- GDCprepare(query = query.esca.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_esca_data'}
...
write.fit(tfit,dt,file='results_esophageal.txt')
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('ESCA','ESCA','ESCA','ESCA'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('ESCA','ESCA','ESCA','ESCA'))
```
...
write.table(combined, file='fdr_ipa_tables/esophageal.tsv', row.names=FALSE, quote=FALSE, sep='\t')
