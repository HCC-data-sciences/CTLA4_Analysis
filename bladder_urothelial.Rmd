---
title: "CTLA-4 Analysis in Bladder Urothelial Carcinoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
```{r load_blca_data, cache=TRUE, include=FALSE, eval=TRUE}
query.blca.hg38 <- GDCquery(project = c("TCGA-BLCA"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.blca.hg38)
raw.counts <- GDCprepare(query = query.blca.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_blca_data'}
...
df.raw.counts <- df.raw.counts %>% dplyr::select(-contains('A0C8-01B'),
                                                 -contains('A13J-01B'),
                                                 -contains('A13I-01B')
                                                 )
...
write.fit(tfit,dt,file='results_bladder.txt')
```
...
```{r clusterProfiler, cache=TRUE, results='asis', fig.height=20, fig.width=15, eval=FALSE}
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('BLCA','BLCA','BLCA','BLCA'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('BLCA','BLCA','BLCA','BLCA'))
```
...
write.table(combined, file='fdr_ipa_tables/bladder.tsv', row.names=FALSE, quote=FALSE, sep='\t')
