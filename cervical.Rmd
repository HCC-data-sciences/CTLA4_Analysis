---
title: "CTLA-4 Analysis in Cervical squamous cell carcinoma and endocervical adenocarcinoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
```{r load_cesc_data, cache=TRUE, include=FALSE, eval=TRUE}
query.cesc.hg38 <- GDCquery(project = c("TCGA-CESC"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.cesc.hg38)
raw.counts <- GDCprepare(query = query.cesc.hg38, summarizedExperiment = FALSE)

```


```{r generate_edge_r_object, dependson='load_cesc_data'}
...
write.fit(tfit,dt,file='results_cervical.txt')
```
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('CESC','CESC','CESC','CESC'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('CESC','CESC','CESC','CESC'))
```
...
write.table(combined, file='fdr_ipa_tables/cervical.tsv', row.names=FALSE, quote=FALSE, sep='\t')
