---
title: "GITR Analysis in Glioblastome Multiforme"
author: "Arjun Mittal"
date: "July 22, 2022"
output:
    html_document:
...
write.fit(tfit,dt,file='results_gitr_gbm.txt')
```
...
no_repeat_tpm <- no_repeat_tpm %>% dplyr::select(-contains('-11'))
no_repeat_tpm <- no_repeat_tpm %>% dplyr::select(-contains('-02'))
```
...
group_res_consensus_tme <- deconvolute(by_group_joined, 'mcp_counter', indications=c('GBM', 'GBM', 'GBM', 'GBM'))
```
...
write.table(combined, file='fdr_ipa_tables/gitr_gbm.tsv', row.names=FALSE, quote=FALSE, sep='\t')
