---
title: "CTLA-4 Analysis in Thyroid Carcinoma"
author: "Arjun Mittal"
date: "July 8, 2022"
output:
    html_document:
...
```{r load_thca_data, cache=TRUE, include=FALSE, eval=TRUE}
query.thca.hg38 <- GDCquery(project = c("TCGA-THCA"),
                            data.category = "Transcriptome Profiling",
                            data.type = "Gene Expression Quantification",
                            #sample.type = "Metastatic",
                            workflow.type = "STAR - Counts"
                        )
GDCdownload(query.thca.hg38)
raw.counts <- GDCprepare(query = query.thca.hg38, summarizedExperiment = FALSE)

```

# Only about 1 fold log change in PDL1 between groups

```{r generate_edge_r_object, dependson='load_thca_data'}
...
df.raw.counts <- df.raw.counts %>% dplyr::select(-contains('-11C'))
...
write.fit(tfit,dt,file='results_thyroid_carcinoma.txt')
```
...
no_repeat_tpm <- no_repeat_tpm %>% dplyr::select(-contains('-11C'))
...
group_res_timer <- deconvolute(by_group_joined, 'timer', indications=c('THCA','THCA','THCA','THCA'))
group_res_consensus_tme <- deconvolute(by_group_joined, 'consensus_tme', indications=c('THCA','THCA','THCA','THCA'))
```
